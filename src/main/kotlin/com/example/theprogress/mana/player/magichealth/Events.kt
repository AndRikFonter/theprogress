...
object Events {
    ...
    fun onPlayerTick(event: PlayerTickEvent.Post) {
        ...
        if (!player.level().isClientSide && player.tickCount % 200 == 0) {
            val magicHealth = player.getData(Attachments.MAGICHEALTH)
            magicHealth.playerTick(player as ServerPlayer)
            // Отправляем ману игроку
            PacketDistributor.sendToPlayer(player, SyncPayload(magicHealth))
        }
    }
}
